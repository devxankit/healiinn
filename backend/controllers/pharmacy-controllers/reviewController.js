...
exports.getReview = asyncHandler(async (req, res) => {
  ...
  // Try cache first
  const cacheKey = generateCacheKey('pharmacy:review', {
...
exports.replyToReview = asyncHandler(async (req, res) => {
  ...
  review.reply = {
    ...
    repliedBy: pharmacyId,
    repliedByRole: ROLES.PHARMACY,
    repliedAt: new Date(),
    repliedByRoleModel: 'Pharmacy',
  };

  await review.save();

  // Invalidate cache
  await deleteCacheByPattern(`pharmacy:review:*:${reviewId}`);
  await deleteCacheByPattern(`pharmacy:reviews:analytics:*`);
...
exports.getRatingAnalytics = asyncHandler(async (req, res) => {
  ...
  const cacheKey = generateCacheKey('pharmacy:reviews:analytics', {
    pharmacyId: pharmacyId.toString(),
    from: from || '',
    to: to || '',
  });
  ...
  const pharmacy = await Pharmacy.findById(pharmacyId).select('rating');
  if (pharmacy && statistics.totalReviews > 0) {
    pharmacy.rating = Number.parseFloat(statistics.averageRating.toFixed(2));
    await pharmacy.save();
  }
