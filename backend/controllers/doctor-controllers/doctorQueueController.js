...
exports.skipPatient = asyncHandler(async (req, res) => {
  ...
  const maxToken = allAppointmentsForMax.length > 0
  ...
  while (targetTokenNumber > originalTokenNumber && fixedTokenNumbers.has(targetTokenNumber)) {
    targetTokenNumber--;
  }
  ...
  if (appointment.queueStatus === 'skipped' && originalTokenNumber === targetTokenNumber) {
    ...
    if (doctor && session.sessionStartTime) {
      appointment.time = calculateTimeFromToken(originalTokenNumber, session.sessionStartTime, avgConsultation);
    }
    
    await appointment.save();

    // Update session and send notifications (same as below)
    // ... (will be handled in the common code below)
  }
  ...
  for (const apt of allMovableAppointments) {
    ...
    while (targetToken >= originalTokenNumber && 
           (fixedTokenNumbers.has(targetToken) || tokensBeingAssigned.has(targetToken))) {
      targetToken--;
    }
    ...
    if (targetToken >= originalTokenNumber && targetToken < apt.tokenNumber) {
      ...
      const timeUpdate = doctor && session.sessionStartTime
        ? { time: calculateTimeFromToken(targetToken, session.sessionStartTime, avgConsultation) }
        : {};
  ...
  if (doctor && session.sessionStartTime) {
    appointment.time = calculateTimeFromToken(appointment.tokenNumber, session.sessionStartTime, avgConsultation);
  }
  ...
  const duplicateTokens = Object.keys(tokenCounts).filter(token => tokenCounts[token].length > 1);
  ...
  if (session.currentToken === originalTokenNumber) {
    ...
    if (nextActiveAppointment && session.currentToken !== nextActiveAppointment.tokenNumber) {
  ...
  try {
    ...
    const reloadedMovedAppointments = movedAppointmentIds.length > 0
    ...
    for (const apt of allMovableAppointments) {
      const reloadedApt = reloadedMovedAppointments.find(r => r._id.toString() === apt._id.toString());
      if (reloadedApt) {
        ...
        if (oldTokenNumber !== newTokenNumber || oldTime !== newTime) {
          try {
            ...
            console.error(`Error sending notification to patient ${apt.patientId}:`, error);
          }
        }
      }
    }
