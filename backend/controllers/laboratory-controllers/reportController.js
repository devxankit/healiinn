...
exports.getDetailedAnalytics = asyncHandler(async (req, res) => {
  ...
  const cacheKey = generateCacheKey('laboratory:reports:analytics', {
  ...
  const [
    ...
    // Lead Statistics (optimized with explicit field projection)
    LabLead.aggregate([
      { $match: leadQuery },
      {
        $project: {
      ...
      },
      {
        $group: {
          ...
          totalLeads: { $sum: 1 },
          completedLeads: {
            $sum: { $cond: [{ $eq: ['$status', LAB_LEAD_STATUS.COMPLETED] }, 1, 0] },
          },
          cancelledLeads: {
            $sum: { $cond: [{ $eq: ['$status', LAB_LEAD_STATUS.CANCELLED] }, 1, 0] },
          },
          totalRevenue: { $sum: '$billingSummary.totalAmount' },
          totalHomeCollectionCharge: { $sum: '$billingSummary.homeCollectionCharge' },
          averageLeadValue: { $avg: '$billingSummary.totalAmount' },
        },
      },
    ]),
    ...
    // Top Tests (by frequency) - optimized with explicit field projection
    LabLead.aggregate([
      { $match: leadQuery },
      {
        $project: {
  ...
  const response = {
    ...
    analytics: {
      leads: {
        ...
        completed: leadData.completedLeads,
        cancelled: leadData.cancelledLeads,
        completionRate:
          leadData.totalLeads > 0
            ? Number.parseFloat(
                ((leadData.completedLeads / leadData.totalLeads) * 100).toFixed(2)
              )
            : 0,
        averageLeadValue: Number.parseFloat((leadData.averageLeadValue || 0).toFixed(2)),
      ...
      revenue: {
        ...
        fromLeads: leadData.totalRevenue || 0,
        homeCollectionCharges: leadData.totalHomeCollectionCharge || 0,
      },
      patients: {
        uniquePatients: patientData.uniquePatients,
        averageLeadsPerPatient:
          patientData.uniquePatients > 0
            ? Number.parseFloat(
                ((leadData.totalLeads / patientData.uniquePatients) * 100).toFixed(2)
              )
            : 0,
      },
      reviews: {
...
exports.exportDetailedReport = asyncHandler(async (req, res) => {
  ...
  // Get laboratory info
  const laboratory = await Laboratory.findById(laboratoryId).select('labName ownerName').lean();
  ...
  const [leadStats, revenueStats, statusBreakdown, patientStats, reviewStats] =
    await Promise.all([
      LabLead.aggregate([
        { $match: leadQuery },
        {
  ...
  const leadData = leadStats[0] || { totalLeads: 0, completedLeads: 0, totalRevenue: 0 };
  ...
  if (format.toLowerCase() === 'pdf') {
    ...
    const fileName = `laboratory-detailed-report-${Date.now()}.pdf`;
    ...
    // Header
    doc.fontSize(20).text('Detailed Laboratory Report', { align: 'center' });
    doc.moveDown();
    ...
    // Lead Statistics
    doc.fontSize(16).text('Lead Statistics', { underline: true });
    doc.fontSize(12);
    doc.text(`Total Leads: ${leadData.totalLeads}`);
    doc.text(`Completed Leads: ${leadData.completedLeads}`);
    doc.text(`Completion Rate: ${leadData.totalLeads > 0 ? ((leadData.completedLeads / leadData.totalLeads) * 100).toFixed(2) : 0}%`);
    doc.moveDown();
    ...
    doc.text(`Revenue from Leads: ₹${leadData.totalRevenue.toFixed(2)}`);
    doc.moveDown();
    ...
    worksheet.getCell('A1').value = 'Detailed Laboratory Report';
    ...
    // Lead Statistics
    let row = 5;
    worksheet.getCell(`A${row}`).value = 'Lead Statistics';
    worksheet.getCell(`A${row}`).font = { bold: true };
    row++;
    worksheet.getCell(`A${row}`).value = 'Total Leads';
    worksheet.getCell(`B${row}`).value = leadData.totalLeads;
    row++;
    worksheet.getCell(`A${row}`).value = 'Completed Leads';
    worksheet.getCell(`B${row}`).value = leadData.completedLeads;
    row++;
    worksheet.getCell(`A${row}`).value = 'Completion Rate';
    worksheet.getCell(`B${row}`).value = `${leadData.totalLeads > 0 ? ((leadData.completedLeads / leadData.totalLeads) * 100).toFixed(2) : 0}%`;
    ...
    const fileName = `laboratory-detailed-report-${Date.now()}.xlsx`;
    const filePath = path.join(process.cwd(), 'backend', 'uploads', 'reports', fileName);
    const uploadsDir = path.dirname(filePath);
    ...
    // CSV format
    const csvRows = [];
    csvRows.push('Detailed Laboratory Report');
    csvRows.push(`Laboratory,${laboratory.labName}`);
    csvRows.push(`Period,${startDate.toLocaleDateString()} - ${endDate.toLocaleDateString()}`);
    csvRows.push('');
    csvRows.push('Lead Statistics');
    csvRows.push('Metric,Value');
    csvRows.push(`Total Leads,${leadData.totalLeads}`);
    csvRows.push(`Completed Leads,${leadData.completedLeads}`);
    csvRows.push(`Completion Rate,${leadData.totalLeads > 0 ? ((leadData.completedLeads / leadData.totalLeads) * 100).toFixed(2) : 0}%`);
    csvRows.push('');
    ...
    res.setHeader('Content-Disposition', `attachment; filename="laboratory-detailed-report-${Date.now()}.csv"`);
...
exports.getPerformanceMetrics = asyncHandler(async (req, res) => {
  ...
  const cacheKey = generateCacheKey('laboratory:reports:performance', {
    laboratoryId: laboratoryId.toString(),
    from: startDate.toISOString(),
    to: endDate.toISOString(),
  });
  ...
  const [
    ...
    // Lead processing metrics (optimized with explicit field projection)
    LabLead.aggregate([
      { $match: leadQuery },
      {
        $project: {
  ...
  const response = {
    ...
    metrics: {
      leadProcessing: {
        ...
        completedLeads: leadData.completedLeads,
        completionRate:
          leadData.totalLeads > 0
            ? Number.parseFloat(
                ((leadData.completedLeads / leadData.totalLeads) * 100).toFixed(2)
