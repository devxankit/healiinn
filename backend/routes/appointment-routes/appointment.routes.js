...
router.get('/patient/appointments/today', protect(ROLES.PATIENT), controller.getTodayAppointments);
router.get('/patient/appointments/upcoming', protect(ROLES.PATIENT), controller.getUpcomingAppointments);

router.get('/doctor/list', protect(ROLES.DOCTOR), controller.listDoctorAppointments);

// Reschedule endpoints (must be after all other routes to avoid conflicts)
router.get('/:appointmentId/available-slots', protect(ROLES.PATIENT), controller.getAvailableSlots);
router.patch('/:appointmentId/reschedule', protect(ROLES.PATIENT), controller.rescheduleAppointment);
