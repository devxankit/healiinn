    ...
    <>
      ...
      <section className={`flex flex-col gap-4 pb-24 ${isDashboardPage ? '-mt-20' : ''}`}>
          ...
          <div className="mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-2xl font-bold text-slate-900">Earnings</h1>
              <p className="mt-0.5 sm:mt-1 text-xs sm:text-sm text-slate-600">View your earnings and income details</p>
            </div>
          </div>

          {/* Main Earnings Card */}
          <div className="mb-4 sm:mb-6 relative overflow-hidden rounded-xl sm:rounded-3xl border border-emerald-100/60 bg-gradient-to-br from-emerald-600 via-emerald-500 to-emerald-600 p-4 sm:p-6 text-white shadow-xl shadow-emerald-500/30">
            ...
            <div className="relative">
              <div className="mb-2 sm:mb-4 flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-xs sm:text-sm font-medium text-white/90">Total Earnings</p>
                  <p className="mt-1 sm:mt-2 text-2xl sm:text-4xl font-bold truncate">{formatCurrency(mockEarningData.totalEarnings)}</p>
                </div>
                <div className="flex h-10 w-10 sm:h-16 sm:w-16 shrink-0 items-center justify-center rounded-lg sm:rounded-2xl bg-white/20 backdrop-blur ml-2">
                  <IoArrowDownOutline className="h-5 w-5 sm:h-8 sm:w-8" />
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Earnings Breakdown */}
          <div className="mb-4 sm:mb-6 grid grid-cols-1 gap-2.5 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-xl sm:rounded-2xl border border-[rgba(17,73,108,0.2)] bg-gradient-to-br from-[rgba(17,73,108,0.05)] via-white to-[rgba(17,73,108,0.05)] p-3 sm:p-5 shadow-sm shadow-[rgba(17,73,108,0.1)]">
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-[#11496c]">Today</p>
              <p className="mt-1.5 sm:mt-2 text-xl sm:text-2xl font-bold text-slate-900">{formatCurrency(mockEarningData.todayEarnings)}</p>
            </div>
            ...
            <div className="rounded-xl sm:rounded-2xl border border-emerald-100/60 bg-gradient-to-br from-emerald-50/90 via-white to-emerald-50/70 p-3 sm:p-5 shadow-sm shadow-emerald-100/50">
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-emerald-700">Month</p>
              <p className="mt-1.5 sm:mt-2 text-xl sm:text-2xl font-bold text-slate-900">{formatCurrency(mockEarningData.thisMonthEarnings)}</p>
            ...
            <div className="rounded-xl sm:rounded-2xl border border-purple-100/60 bg-gradient-to-br from-purple-50/90 via-white to-purple-50/70 p-3 sm:p-5 shadow-sm shadow-purple-100/50">
              <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-purple-700">Year</p>
              <p className="mt-1.5 sm:mt-2 text-xl sm:text-2xl font-bold text-slate-900">{formatCurrency(mockEarningData.thisYearEarnings)}</p>
            </div>
          </div>

          {/* Filter Tabs */}
          <div className="mb-3 sm:mb-4 flex gap-1.5 sm:gap-2 overflow-x-auto pb-2 scrollbar-hide [-webkit-overflow-scrolling:touch]">
            <button
              type="button"
          ...
          {/* Earnings List */}
          <section>
            <h2 className="mb-3 sm:mb-4 text-base sm:text-lg font-bold text-slate-900">Earning History</h2>
            <div className="space-y-2 sm:space-y-3">
              {filteredEarnings.length === 0 ? (
                <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-8 sm:p-12 text-center shadow-sm">
                  <IoArrowDownOutline className="mx-auto h-12 w-12 sm:h-16 sm:w-16 text-slate-300" />
                  <p className="mt-3 sm:mt-4 text-sm sm:text-base font-semibold text-slate-600">No earnings found</p>
                  <p className="mt-1 text-xs sm:text-sm text-slate-500">Your earnings will appear here</p>
                </div>
              ) : (
                filteredEarnings.map((earning) => (
                  ...
                  >
                    <div className="flex items-start gap-2.5 sm:gap-4">
                      <div className="flex h-9 w-9 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-lg sm:rounded-xl bg-emerald-100">
